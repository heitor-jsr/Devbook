...
func (suite *TestUserControllerSuite) TestCreate() {
	...
	t.Run("Success on creating user", func(t *testing.T) {
		...
		assert.Equal(t, uint64(4), lasInsertId)
	})
...
func (suite *TestUserControllerSuite) TestFollowUser() {
	t := suite.T()
	...
	t.Run("Fail when the id from token is equal from the id from URL", func(t *testing.T) {
		...
		expectedResponseBody := struct {
			...
			Erro: "Não pode seguir a si mesmo.",
		}

		expectedResponseMessage := "Não pode seguir a si mesmo."
